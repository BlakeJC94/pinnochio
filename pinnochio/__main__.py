...
def load_uv_dependencies() -> tuple[DependencyGroups, tomlkit.TOMLDocument]:
    """Load dependencies and return both the parsed groups and the raw TOML document."""
...
def check_group_overlaps_match(
    ...
    if drift:
        ...
        if fix:
            print(
                "Note: Automatic fixing of version drift is not implemented."
            )
            print(
...
def _add_upper_bound(pin: str) -> str:
    ...
    Converts 'package>=x.y.z' to 'package>=x.y.z,<x+1' where x is the major version.
    """
    ...
    return f"{package_name}>={major}.{minor}.{patch},<{major}.{int(minor)+1}.0"
